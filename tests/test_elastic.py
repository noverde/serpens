...
class TestElastic(unittest.TestCase):
    ...
    def test_setup(self):
        ...
        for processor in processors:
            self.assertTrue(processor in apm_processors)
    ...
    def test_setup_with_processors(self):
        ...
        value = "elasticapm.processors.sanitize_http_headers"
        self.os_mock.environ["ELASTIC_APM_PROCESSORS"] = value

        elastic.setup()

        processors = self.os_mock.environ.get("ELASTIC_APM_PROCESSORS").split(",")

        self.assertEqual(len(processors), 3)
        for processor in processors_expected:
            self.assertTrue(processor in processors)
